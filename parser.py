...
class Parser:
    ...
    def program(self):
        if not self.expect(Token.KEYWORD, lexeme="HAI", consume=False, required=False): self.accept(self.statement)
        self.expect(Token.KEYWORD, "HAI")
        self.expect(Token.NEWLINE, required=False)
        self.expect(Token.NUMBAR, required=False)
        self.expect(Token.NEWLINE, required=False)
        
        # Function Declarations
        Log.yell("FSTART")
        while not self.expect(Token.KEYWORD, lexeme="WAZZUP", consume=False, required=False):
            Log.yell("Add function")
            self.whitespacer()
        ...
        while not self.expect(Token.KEYWORD, lexeme="BUHBYE", required=False):
            if not self.expect(Token.NEWLINE, consume=False, required=False ):
    ...
    def expr(self): 
        ...
        if got:= self.expect(Token.NEWLINE, required=True):
            return Token.NEWLINE
        
        # if self.expect(Token.NEWLINE, consume=False, required=True): self.next() #skip newline
        return None 
    ...
    def statement(self):
        ...
        if op := self.expect(Token.OPERATOR, consume=False, required=False):
            Log.i(f"Got operation: {self.current.lexeme}")
            it = self.accept(self.expr, err_msg=f"Error evaluating expression")
            self._set("it", it)
            print(f"WAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA {it}")
            return it
    ...
    def print(self):
        Log.e(f"{self.current} <- first?")
        first = self.accept(self.expr, f"Expected literal or <expr>, got {self.current.lexeme}")
        operands = [self.cast(first, "YARN")]
        
        Log.e(f"{self.current} <- next?")
        try:
            if not self.expect(Token.NEWLINE, required=False): #Immediate newline
            ...
            while not self.expect(Token.NEWLINE, required=False):
                op = self.accept(self.expr, err_msg="Expected: <expr>")
                operands.append(self.cast(op, "YARN"))
            pass #In case of another keyword
    ...
    def whitespacer(self):
        while self.expect(Token.NEWLINE, consume=False, required=False):
            if not self.expect(Token.NEWLINE, consume=False): break
            self.next()
