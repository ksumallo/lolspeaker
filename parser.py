...
class Parser:
    ...
    def program(self):
        ...
        while not self.expect(Token.KEYWORD, lexeme="KTHXBYE", consume=False, required=False):
            ...
            if self.current.type == Token.KEYWORD: self.accept(self.statement, err_msg="Statement not recognized")
                ...
                elif self.expect(Token.KEYWORD, lexeme="WTF?", consume=False, required=False): self.switch_case(expr_val)

    def expr(self): 
        Log.d(f"EXPRESSION: {self.current}")
        # Just a temporary patch for assigning
        ...
        if got := self.expect(Token.YARN, required=False):
            if got[0] == '"' or got[0] == "'": return str(got[1:-1])
            else: return str(got)
    ...
    def statement(self):
        ...
        if self.expect(Token.KEYWORD, lexeme="WTF?", required=False):
            self.accept(self.switch_case, err_msg="Error occurred while in WTF?")
    ...
    def print(self):
        ...
        if self.expect(Token.KEYWORD, lexeme="AN", required=False):
            self.accept(self.print, err_msg="Oh no!")
    ...
    def assign(self, prev_var=None):
        if prev_var == None: var = self.expect(Token.IDENTIFIER)
        else: var = prev_var

        self.expect(Token.KEYWORD, lexeme="R")
        
        Log.d(f"Assigning new value to {var}")
    ...
    def cond(self, parameter):
        ...
        while not self.expect(Token.KEYWORD, lexeme="OIC", consume=False, required=False):
            if self.expect(Token.KEYWORD, lexeme="YA RLY"):
                while True:
                    if self.expect(Token.KEYWORD, lexeme="NO WAI", consume=False, required=False):
                        break
                    if self.expect(Token.KEYWORD, lexeme="OIC", consume=False, required=False):
                        self.next()
                        return
                    if self.expect(Token.KEYWORD, lexeme="MEBBE", consume=False, required=False):
                        self.next()
                        is_true = self.accept(self.expr, err_msg="Expression not recognized")
                        if is_true:
                            parameter = True
                            while not self.expect(Token.KEYWORD, lexeme="MEBBE", consume=False, required=False) and self.expect(Token.KEYWORD, lexeme="NO WAI", consume=False, required=False):
                                if self.expect(Token.KEYWORD, lexeme="MEBBE", consume=False, required=False) or self.expect(Token.KEYWORD, lexeme="NO WAI", consume=False, required=False):
                                    self.next()
                                    break
                                if self.current.type == Token.KEYWORD:
                            ...
                            elser = False
                            while not self.expect(Token.KEYWORD, lexeme="MEBBE", consume=False, required=False):
                                if self.expect(Token.KEYWORD, lexeme="NO WAI", consume=False, required=False):
                                    elser = True
                                    break
                                else:
                    ...
                    if parameter:
                        ...
                        self.next()
            if self.expect(Token.KEYWORD, lexeme="NO WAI", required=False):
                Log.e(parameter)
                while not self.expect(Token.KEYWORD, lexeme="OIC", consume=False, required=False):
                    if not parameter:
                        if self.current.type == Token.KEYWORD:
    ...
    def switch_case(self, parameter):
        self.next()        
        ...
        while not gtfoed:
            if self.expect(Token.KEYWORD, lexeme="OIC", consume=False, required=False):
                        Log.e("OIC encountered.")
                        break
            
            if self.expect(Token.KEYWORD, lexeme="GTFO", consume=False, required=False): #SPAGHETTI
                            Log.d("GTFO encountered, exiting case.")
                            if case_matched: gtfoed = True
                            self.next()
                            continue  # Exit the loop when 'GTFO' is encountered
            ...
            if self.expect(Token.KEYWORD, lexeme="OMG", required=False):
                ...
                try: parameter = self.cast(parameter, self.typeof(case_value)) 
                except: Log.e("Cannot be casted to respective data type")
                Log.i(f"Case value: {parameter}, IT value: {case_value}")
                if (case_value == parameter):
                    Log.i(f"Case matched: {case_value} == {parameter}")
                    case_matched = True
                    # If case matched, execute the statements inside the case block
                    while True:
                        if self.expect(Token.KEYWORD, lexeme="GTFO", consume=False, required=False):
                            Log.d("GTFO encountered, exiting case.")
                            if case_matched: gtfoed = True
                            self.next()
                            break  # Exit the loop when 'GTFO' is encountered
                        if self.expect(Token.KEYWORD, lexeme="OMG", consume=False, required=False) or self.expect(Token.KEYWORD, lexeme="OMGWTF", consume=False, required=False) or self.expect(Token.KEYWORD, lexeme="OIC", consume=False, required=False):
                            Log.d("Next case!")
                            break
            ...
            # If no cases matched, check for the default case: OMGWTF
            elif self.expect(Token.KEYWORD, lexeme="OMGWTF", required=False):
                ...
                while True:
                    ...
                    if self.expect(Token.KEYWORD, lexeme="OIC", consume=False, required=False):
                        ...
                        gtfoed = True #considered as gtfoed since this is the end of the switch case
                        break
                    if self.current.type == Token.KEYWORD:
                            self.accept(self.statement, err_msg="Statement not recognized")
                    else:
        ...
        while True:
            if self.expect(Token.KEYWORD, lexeme="OIC", consume=False, required=False):
                Log.e("Switch-case ended with OIC.")
                self.next()
